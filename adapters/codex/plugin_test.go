...
const reviewerProxyEvidenceGuidance = "A criterion is also wrong when the only evidence available for it is a proxy for what it asked. A criterion requiring a check on how an LLM agent routes, what it decides, or how it behaves is the standing instance"

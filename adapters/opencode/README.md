...
| Local writer `localOpenCodeRoleDocSpecs` | Before, local OpenCode profiles used static model and variant suggestions. After, every role uses the live catalog, retains only its committed model as the non-catalog compatibility choice, and accepts only advertised variants for a selected live model. |
| Native setup pagination (`question.Pagination` / `PaginateOptions`) | Before, the core emitted no page contract and a one-answer domain could reach a setup skill as a host-invalid one-option select. After, one shared deterministic page sequence serves Claude, Codex, and OpenCode, with two or three mutually exclusive answer/navigation options per page and every answer appearing exactly once without free text. A one-answer domain — such as init with one live model and no retained value — is model plus cancel. Configure and configure-local may append an unavailable committed model to a one-entry live catalog; that two-answer domain is both models with no cancel. |

...
| Native setup pagination (`question.Pagination` / `PaginateOptions`) | New. The core emits one shared deterministic page sequence for Claude, Codex, and OpenCode. Every page has two or three mutually exclusive answer/navigation options; a one-model catalog is model plus cancel, and larger catalogs include every model exactly once without free text. Each setup skill presents emitted pages rather than inventing its own split. |

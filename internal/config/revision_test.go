...
func TestRevisionPreservesShippedV080OpenCodeConfiguration(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "..", ".orchestrator", "config.toml"))
	if err != nil {
		t.Fatal(err)
	}
	cfg, err := Parse(data)
	...
	got, err := Revision(cfg)
	if err != nil {
		t.Fatal(err)
	}
	if got != cfg.ConfigRevision {
		t.Errorf("Revision = %q, shipped v0.8.0 revision = %q", got, cfg.ConfigRevision)
	}
}
